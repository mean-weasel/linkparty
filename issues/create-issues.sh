#!/usr/bin/env bash
# Creates all review issues in GitHub using the gh CLI.
# Usage: cd linkparty && bash issues/create-issues.sh
#
# Prerequisites: gh auth login

set -euo pipefail

REPO="neonwatty/linkparty"
...
create \
  "Add component tests for untested UI components" \
  "test" \
  "$(cat <<'EOF'
...
create \
  "Add E2E tests for image upload, offline/sync, and keyboard navigation" \
  "test" \
  "$(cat <<'EOF'
...
create \
  "Add unit tests for hooks: useImageUpload, useNotifications, useFocusTrap" \
  "test" \
  "$(cat <<'EOF'
...
create \
  "Fix RLS policy allowing anonymous read access to queue items" \
  "security" \
  "$(cat <<'EOF'
...
create \
  "Add brute-force throttling on party join by code" \
  "security" \
  "$(cat <<'EOF'
...
create \
  "Fix CORS header inconsistency in edge function error responses" \
  "security" \
  "$(cat <<'EOF'
...
create \
  "Add per-code rate limiting to prevent party code enumeration" \
  "security" \
  "$(cat <<'EOF'
...
create \
  "Improve accessibility: focus-visible styles, keyboard drag-and-drop, non-color indicators" \
  "accessibility" \
  "$(cat <<'EOF'
...
create \
  "Add offline queue buffering and sync-on-reconnect" \
  "enhancement" \
  "$(cat <<'EOF'
...
create \
  "Configure push notification delivery (APNs / FCM)" \
  "enhancement" \
  "$(cat <<'EOF'
...
create \
  "Enhance TV mode: auto-advance timer and queue count indicator" \
  "enhancement" \
  "$(cat <<'EOF'
...
create \
  "Add loading skeletons for queue, members, and now-showing sections" \
  "enhancement" \
  "$(cat <<'EOF'
...
create \
  "Add search and filter for the queue" \
  "enhancement" \
  "$(cat <<'EOF'
...
create \
  "Add bulk operations: multi-select and bulk delete" \
  "enhancement" \
  "$(cat <<'EOF'
...
create \
  "Add party preview before joining" \
  "enhancement" \
  "$(cat <<'EOF'
...
create \
  "Add configurable party settings (queue limits, rate limits)" \
  "enhancement" \
  "$(cat <<'EOF'
